...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true) * 0.7f;
    ...
    const ImVec2 label_pos(check_bb.Max.x + style.ItemInnerSpacing.x + 1.0f,
        check_bb.Min.y - label_size.y * 0.1f); 
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float height = g.FontSize * 0.6f;
    const ImRect frame_bb(pos + ImVec2(0, label_height), pos + ImVec2(w, height + label_height));
    const ImRect total_bb(pos, pos + ImVec2(w, height + label_height));
    ...
    it_anim->second.Frame = ImLerp(it_anim->second.Frame, colFrame, 1.0f / 0.005 * ImGui::GetIO().DeltaTime);
    it_anim->second.Grab = ImLerp(it_anim->second.Grab, colGrab, 1.0f / 0.005 * ImGui::GetIO().DeltaTime);
