...
#define IMADD_ANIMATIONS_SPEED 0.1f // Second
...
namespace ImAdd
{
	...
	void	Separator(const char* fmt, ...);

	bool    Button(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags = 0);
	bool	TabButton(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags);
	...
	bool	ColorEdit3(const char* label, float col[4]);
}
